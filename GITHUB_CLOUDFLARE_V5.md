...
This archive is the complete V5 source. It preserves the public design,
account screenshots, calculators, services, links and `/admin` in one
...
3. Replace the older V5 files when GitHub asks.
4. Commit to the `main` branch.

The archive intentionally omits `node_modules/`, `dist/`, `.wrangler/`, local
runtime caches and `.git/`. Cloudflare rebuilds them from the lockfile.
...
The included `wrangler.jsonc` now runs `npm run build` automatically before
every Wrangler deployment. This means `npx wrangler deploy` creates
`dist/server/index.js` even when Cloudflare does not run a separate build
field.

The shell helpers are invoked through `bash`, so GitHub browser uploads and
Windows ZIP extraction no longer cause the previous `sites-env.sh: Permission
denied` failure.

The configuration also keeps the existing `SALESMAN_DATA` KV namespace.
Verify that the namespace ID in `wrangler.jsonc` still belongs to your
Cloudflare account before the first deployment. If it was replaced, update
only that ID.
...
Sheets tabs; if a sheet is unavailable, the last working prices remain in
place.

## Visitor analytics

V5 records first-party page-view and click events. Open `/admin` and use the
Analytics section after logging in to see the total recorded visits, last
seven days, event types and visited paths. These figures start accumulating
after this V5 Worker is deployed and use `SALESMAN_DATA`.
...
canonical URL and sitemap if the hostname differs from the current files.
