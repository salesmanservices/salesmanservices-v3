...
function renderAccounts(){
 ...
 <article class="account-card">
  ...
  <div class="account-info">
   ...
   ${reservationMarkup(a)}
   <a class="button primary ${isReserved(a)?"disabled-reserved":""}" href="${isReserved(a)?"#":accountActionHref(a)}" ${!isReserved(a)&&isDiscordOnly(a) ? `target="_blank" rel="noopener"` : ""}>${isReserved(a)?"Reserved":accountActionLabel(a)}</a>
  </div>
 </article>`).join("");
...
(async()=>{await loadManagedSiteData();buildAccounts();renderSoldAccounts();buildSkill();buildQuests();buildMinigames();buildCapes();refreshReservationCountdowns();setInterval(refreshReservationCountdowns,1000)})();
...
(function(){
  ...
  if(launcher){
    launcher.addEventListener('click',()=>{
      ...
      else window.open('https://discord.gg/xDSvKT3ThQ','_blank','noopener');
    });
  }
